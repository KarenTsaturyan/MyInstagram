...
const ignoreChatSpaces = (store) => (next) => (action) =>{
   if((action.type === 'messages/addText'  && action.payload.text.replaceAll(' ', '')) || action.type !== 'messages/addText'){
    next(action)
   }
  }
...
const store = configureStore({
    ...
    middleware: (getDefaultMiddlewares)=>{
         return [...getDefaultMiddlewares({
          ...
          }),ignoreSpaces, allLowerCase, ignoreChatSpaces]
    }
})
